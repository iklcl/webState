...
function addLayerPropertyBox(e){
	...
	setTimeout(function(){
		popupRight.setLngLat(e.features[0].geometry.coordinates).setHTML('<table class="popTable">' + '<tr>' +
                        '<td>名称</td>' +'<td>'+
						'<input type="text" placeholder="我的标记" id="addLayerText" />'
						+'</td>' +
                        '</tr><tr>' +
                        '<td>类型</td>' +
                        '<td>'+
						'<select id="addLayerType" ><option value="symbol">文本</option></select>'
						+'</td>' +
                        '</tr><tr>' +
                        '<td>颜色</td>' +
                        '<td>'+ 
						'<input type="color" id="addLayerColor" />'
						+'</td>' +
                        '</tr><tr>' +
                        '<td>大小</td>' +
                        '<td>'+
						 '<input type="text" placeholder="请输入大于0的整数" id="addLayerSize" />'
						 +'</td>' +
                        '</tr>' +
                        '</table>'+
						'<div class="comf">'+
						'<div class="btn-group btn-group-sm" role="group" >'+
							  '<button type="button" class="btn btn-success" onclick="addLayerProperty()">添加</button>'+
							  '<button type="button" class="btn btn-default" onclick="{popupRight.remove()}">取消</button></div>'+
						'</div>').addTo(map);},10);
...
function addLayerProperty(){
	...
	let name = txtAddLayerName.value === ""? "我的标记":txtAddLayerName.value ;
